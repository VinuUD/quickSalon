    ...
    function (responseJson) {
      ...
      $.each(responseJson, function (index, service) {
        ...
        serviceDetails.push({
          ...
          time: new Time(
            Math.floor(service.timeTaken / 60),
            service.timeTaken % 60
          ),
        });
      });
    }
...
$("#servicedropdownlist").change(function () {
  ...
  // alert(timeTaken)
  $.get(
    "http://localhost:8080/quickSalon_war_exploded/serviceProvider/serviceProviderList?sid=" +
      sid,
    function (responseJSON) {
...
function addAppointment() {
  ...
  appointmentArray.forEach((appointment, index) => {
    tableContent +=
      ...
      "</td><td>" +
      appointment.time +
      "</td><td><button onclick='removeApt(" +
      index +
      ")'>x</button></td></tr>";
  });
...
function placeAppointment() {
  ...
  appointmentArray.forEach((appointment) => {
    $.post(
      "http://localhost:8080/quickSalon_war_exploded/serviceProvider/spappointments",
      {
